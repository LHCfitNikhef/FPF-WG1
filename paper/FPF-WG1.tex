...
\begin{center}
  ...
  \vspace{1.1cm}
  {\small
Max Fieg$^{1}$, Tommaso Giani$^{2,3}$, Peter Krack$^{2,3}$  Giacomo Magni$^{2,3}$, Toni M\"akel\"a$^{4}$, \\[0.1cm]
...
Proton-proton collisions at the LHC produce an intense collimated flux of neutrinos
characterised by the largest energies (up to several TeV) achieved in laboratory experiments.
%
Until recently, these neutrinos escaped detection and hence were lost for physics.
%
The observation in March 2023 of LHC neutrinos by the FASER$\nu$ and SND@LHC experiments
heralds the beginning of a ``LHC neutrino era'' offering unprecedented scientific opportunities
for particle and astroparticle physics both within the Standard Model and beyond it.
%
Here we assess the expected impact of LHC neutrino deep-inelastic scattering (DIS) structure functions
on the quark and gluon substructure of nucleons and heavy nuclei.
%
In addition to projections for FASER$\nu$ and SND@LHC based on the full Run III luminosity,
we also consider the experiments entering the proposed
 Forward Physics Facility (FPF) to operate concurrently with the HL-LHC.
%
 We generate pseudo-data for inclusive and charm DIS structure functions for these various experiments,
model their  systematic uncertainties, and quantify their impact
on proton and nuclear PDFs by means of both  Hessian profiling within {\sc\small xFitter}
and using the NNPDF framework.
%
We find the LHC neutrino scattering, specially
for the high-statistics  experiments at the FPF, provides  stringent constraints
concerning the separation between quark and antiquark PDFs of different flavours.
%
Our analysis demonstrates that detection and analysis of LHC far-forward neutrinos  effectively
extends the LHC with a charged-current counterpart of the Electron-Ion Collider (EIC)
with comparable kinematic reach and hence realises a ``Neutrino-Ion Collider'' at CERN.
...
% Analysis settings, theory calculations, xfitter vs NNPDFs, yadism/eko/pineappl
% Fitting vs profiling
\input{sec-analysis}

% REsults for the proton PDFs, using NNPDF and PDF4LHC21/xfitter
\input{sec-protonPDFs}

% REsults for the nuclear PDFs, using NNPDF and EPPS21/xfitter
\input{sec-nuclearPDFs}
