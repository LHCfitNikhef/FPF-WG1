...
a diagonal CKM matrix and no heavy quark mass effects.
 ...
 \bea
 E_\nu &=& E_h + E_\ell \, , \nonumber \\
 Q^2 &=& 4 ( E_h + E_\ell) E_\ell \sin^2 \lp \theta_\ell/2\rp \, ,\\
...
\begin{figure}[t]
    \centering
\includegraphics[width=0.8\textwidth]{plots/Nevent_FASERv2_14.pdf}
\caption{\small The event yields per bin $N_{\rm ev}^{(i)}$,  Eq.~(\ref{eq:event_yields}),
  for muon neutrinos at the FASER$\nu$2 detector, restricted
  to the DIS region defined by Eq.~(\ref{eq:DISconditions}).
  %
  We present results binned in $x$ and $Q^2$ integrated over neutrino energies.
  %
  The red dots indicate the weighted bin center.
  %
...
one needs to also estimate the systematic uncertainties associated to the reconstruction
of the final state leptonic and hadronic variables listed in Table~\ref{tab:FPF_experiments}.
%
Event which would be classified in a given bin in the case
of a perfect detector may end up being classified into a different bin in the presence of systematic
