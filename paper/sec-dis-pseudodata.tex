...
\begin{figure}[t]
    ...
    \caption{The kinematic coverage in the $(x,Q^2)$ plane of muon-neutrino scattering
      ...
      %
      Bins with less than 30 events have been excluded.
      %
...
\be
\delta_{\rm sys}^{(E_\ell)} N^{(i)} = \Big| \la {N}^{(i)}\ra_{E_\ell-{\rm smeared}} -N_{\rm ev}^{(i)}\Big| \, .
\ee
...
\label{eq:covmat_definition_v2}
 {\rm cov}_{ij} = \delta_{ij} \lp \delta^{\rm (stat)}  N_{\rm ev}^{(i)}\rp^2
+ \lp f_{\rm corr}\rp^2\sum_{k=1}^{n_{\rm sys}} \lp f_{\rm red}^{(k)}\rp^2\lp \delta_{\rm sys}^{(k)} N_{\rm ev}^{(i)} \rp^2
\, ,\qquad i,j=1,\ldots,N_{\rm bin} \, .
\ee
...
%
Furthermore, Here $f_{\rm corr}$ represents an effective correction factor that accounts for the fact that data with correlated
...
for the systematic uncertainty reduction $f_{\rm red}^{(k)}$ with respect to the baseline settings.   
...
This ensures that the generated pseudo-data is consistent with the prior PDF
 ...
 %
 As mentioned above, $f_{\rm red}^{(k)}$ is a reduction factor that accounts possible
 ...
 %
 Here we consider two scenarios for the systematic errors: conservative  $f_{\rm red}^{(k)}=1$, so
 no improvement as compared to current projected performance, and  $f_{\rm red}^{(k)}=0.5$, a factor
 two improvement for all relevant sources.
 %
...
%  \be
%\label{eq:covmat_definition_v2}
% {\rm cov}_{ij} = \delta_{ij} \lp \delta^{\rm (stat)} \mathcal{O}_i^{\rm (th)} \rp^2
% + \delta_{ij}\sum_{k=1}^{n_{\rm sys}}
% \lp f_{\rm red}^{(k)}\rp^2\lp \delta_{\rm sys}^{(k)} \mathcal{O}_i^{\rm (th)} \rp^2
%\, ,\qquad i,j=1,\ldots,N_{\rm bin} \, ,
%\ee
...
%%
%A tolerance factor $T = \sqrt{\Delta \chi^2}=3$ is adopted for the proton
%PDF profiling, corresponding approximately to the average tolerance
%used in the CT18~\cite{Hou:2019efy} and MSHT20~\cite{Bailey:2020ooq} determinations,
%the two Hessian sets entering the PDF4LHC21 combination.
%The same choice was adopted in the HL-LHC projections of~\cite{AbdulKhalek:2018rok},
%and a common value is used for the Hessian profiling of EPPS21.
%%
