...
adding up to a mass of 1.1 tons with a fiducial volume of $20~\rm{cm} \times 25~\rm{cm} \times 30~{\rm cm}$ {\color{blue} [MF: Is 30 cm correct? The length I have is 100 cm. Looking at \cite{FASER:2023zcr}, I think that the final length of tungsten might be closer to 80cm = 730 layers * 1.1mm/layer]}.
%
...
%
With a total fiducial volume of 830 kg {\color{blue}[ MF: and length of 35 cm]}, it is composed by tungsten plates,
...
of final-state kinematics is assumed.
 ...
 %
 The target region of the detector, where the neutrino interactions take place, is made of thin sensitive layers interleaved with tungsten plates, for a total mass of 5 tons {\color{blue} [MF: and fiducial length of 50 cm]}.
 %
...
muons would be measured in the FASER2 spectrometer
 ...
 With an expected fiducial (active) mass of 10 tons (30 tons) {\color{blue} [MF: and length 3 m]}, FLArE will
...
implemented in Eq.~(\ref{eq:event_yields_calculation}) {\color{blue} [MF: There is a discrepancy beyond this. Our integrated cross sections come out a bit smaller. The total cross sections are very sensitive to kinematic cuts. TR and I have talked about this before and I will message] }, when extrapolated to a single bin
...
\begin{figure}[t]
    ...
    \caption{\small The event yields per bin $N_{\rm ev}^{(i)}$,  Eq.~(\ref{eq:event_yields}), for muon neutrino
      scattering at the
      FASER$\nu$, AdvSND, FASER$\nu$2, and FLArE-100  experiments.
            %
      Selected events are restricted to the DIS region Eq.~(\ref{eq:DISconditions})
      and only bins with $\ge 30$ events are retained.
	%
...
\begin{figure}[t]
    \centering
    \includegraphics[width = 0.85\textwidth]{plots/KIN_DIS_FPF.pdf}
    \caption{The kinematic coverage in the $(x,Q^2)$ plane of muon-neutrino scattering
      ...
      %
      Bins with less than 30 events have been excluded.{\color{blue}[ MF: Currently, not 30 -  10 for Run3 expt's and 100 for HL-LHC expts]}
      %
...
%
Since we discard bins with less than 30 events{\color{blue} [MF: not 30. Currently 10 for Run3, 100 for HL-LHC]}, the fractional statistical uncertainty
...
%
By comparing $\mathcal{D}_0$ with the total number of events as calculated by Eq.~(\ref{MCintegration}), we can normalize the distribution to produce the expected yield.{\color{blue} [MF:changed some wording here ]}
...
\ee
{\color{blue} [MF:Changed some wording here and the equation slighlty to what we do]}
...
reaching up to $\sim 40\%$.{\color{blue} [MF: Sometimes uncertainties can be huge more than 100\%. We ignore these bins. Fig 2.4 will also be updated soon probably, and here we will see some larger uncerainties due to the updated cross sections.]}
%
