...
to be the mean of the absolute difference between the number of events in this bin for the smeared samples $\{\mathcal{D}_k\}$  and the number of events in this bin for ${\mathcal{D}_0}$ which also serves as a normalization to produce a fractional uncertainty:
\be
\delta_{\rm sys}^{(E_\ell)} =\la \left|  \frac{{N}^{(i)}_{E_\ell-{\rm smeared},k} -N_0^{(i)}}{N_0^{(i)}}\right|\ra \, .
\ee
The systematic uncertainty in event yield caused by $\delta E_\ell$ is then $\delta_{\rm sys}^{(E_\ell)} N_{\rm ev}^{(i)}$.
%
