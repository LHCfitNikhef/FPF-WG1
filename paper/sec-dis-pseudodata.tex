...
$ N_{\rm ev}^{(i)(k)}$.
%
We take the uncertainty to be the absolute difference betweeen the mean number of events in the bin for the smeared samples samples, ${\overline{N}}^{(i)}~$ and$~ N_{\rm ev}^{(i)}$ as calculated from Eq. \ref{MCintegration}, {\it{i.e.}}  $\delta N^{(i)} = |{\overline{N}}^{(i)} -N_{\rm ev}^{(i)}|$ .
%
Furthermore, 
