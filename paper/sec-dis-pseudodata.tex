...
we can normalize the sampled $\mathcal{D}_0$ distribution with a factor $\hat{f}$ to reproduce the expected yield.
%
...
We define the uncertainty associated
to a given systematic source, say $\delta E_\ell$,
to be the mean of the absolute difference between the number of events in this bin for the smeared samples $\{\mathcal{D}_k\}$ normalized to $\hat{f}$  and the central prediction for the number of events
$N_{\rm ev}^{(i)}$ as calculated from Eq.~(\ref{eq:event_yields_calculation}):
\be
\delta_{\rm sys}^{(E_\ell)} N_{\rm ev}^{(i)} =\la \Big|  \hat{f} {N}^{(i)}_{E_\ell-{\rm smeared}} -N_{\rm ev}^{(i)}\Big|\ra \, .
\ee
