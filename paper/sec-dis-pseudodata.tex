...
\begin{figure}[t]
    \centering
    \includegraphics[width = 0.6\textwidth]{plots/KIN_DIS_FPF.pdf}
...
The event yields displayed in Fig.~\ref{fig:fasernu2_muon} 
determine the associated statistical uncertainty in each bin by means of
\be
...
This ensures that the generated pseudo-data is consistent with the prior PDF
 ...
 together with the corresponding covariance matrix
  \be
\label{eq:covmat_definition_v2}
...
are contained in the nuisance parameter vectors $b^{(\textrm{pd})}$ and $b^{(\textrm{th})}$, respectively, and the total uncorrelated uncertainty is $\delta^{{\rm (stat)}}\mathcal{O}_i^{\rm (th)}$.
