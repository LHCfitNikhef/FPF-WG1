...
\begin{table}[!t]
  ...
  \midrule
  $\chi^2/n_{\rm dat}$ &  \multirow{4}{*}{Global}  & 1.170 &  1.161  & 1.145  \\
  ...
  & FPF (total)                 &  1.301 / 0.958 &   1.192    &   0.919   \\
  \bottomrule
\end{tabularx}
\vspace{0.2cm}
\caption{\small Statistical estimators for the NNPDF4.0 NNLO
  ...
  For the baseline fit we present both the value computed with only statistical error
  and also adding the systematics so that it can be compared with both fits. 
...
benefit theoretical predictions for $W$ and $Z$ production at the LHC.
%
...
Fig.~\ref{fig:profiling_FPF_nuclear} displays a similar comparison as that of
 ...
 on nPDFs.
 %
 Assuming that the trend of a direct refit is similar to the profiling results in the present work, but that the constraining power improves, the inclusion of FPF data could enhance the precision of quark PDFs in tungsten, hence providing insight into quark shadowing effects.
 %
